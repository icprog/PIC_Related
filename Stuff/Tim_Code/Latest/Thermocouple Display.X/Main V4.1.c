...
//Main4.0.c sent 10 bytes of data across the SPI bus to the seven segment pic Main 4.1.c sends 5 bytes
//#include    <htc.h>
#include <xc.h>
//#include <pic16f726.h>
...
//    long int working_register = 0;
    ...
    __delay_us(30); //changed from 100 to 20 to increase speed
    RA1 = 1;
    while (x <= 3){  //number is equal to amount of thermocouple inputs
        RA2 = 0; //increment SS to point to first TC chip
        __delay_us(100);// 100us seems to be the lowest
//        RA2 = 1; //increment SS
        byte1= SPI_TRANSFER(byte_to_send);
        byte2 = SPI_TRANSFER(byte_to_send);
        byte3 = SPI_TRANSFER(byte_to_send);
...
main(void) {
    ...
    spi_data[5] = 0xaa; //last byte in communication sequence
    spi_data[6] = 0x48; 
    spi_data[7] = 0x48; 
    spi_data[8] = 0x48; 
    spi_data[9] = 0x48; 
