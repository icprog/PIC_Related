...
void (SPI_DATA_HANDLING)(char *data_pointer){
    ...
    if (*data_pointer > 1){
      ...
      if (spi_timer > 200){    //adjust this timer to clear missed info
            spi_timer = 0;
            *data_pointer = 1;
            if (RA3 == 1){  //diagnostic
                RA3 = 0;    //diagnostic
...
void (LOOK_FOR_TEMP_DATA)(char *data_pointer){
    ...
    int x = 1;
    int temp = 0;
    if  (*data_pointer > 10){   //check to see if all 10 Bytes have been received
        if (RA1 == 1){  //diagnostic
            RA1 = 0;    //diagnostic
          }             //diagnostic
...
void (DISPLAY_TEMP)(int sensor_number){
    ...
    if (temp_array[sensor_number] > 0x1FFF){         //Look for sign bit
        temp_array[sensor_number] = temp_array[sensor_number] & 0x1FFF;   //AND out sign bit
        //Add code here. Probably same as below just for negative. maybe make into if and else
    }
...
//////////////////////////////////////////////////
        cycles = 0;
        a++;
        if (a >= 7){        //adjust for amount of temps to cycle through
            a = 1;
        }
        temp = a;
