...
#define CLK TRISCbits.TRISC5
#define DIN TRISCbits.TRISC4
#define DC TRISCbits.TRISC3
#define CE  TRISCbits.TRISC6
#define RST TRISCbits.TRISC7
...
font that is 5 pixels wide and 8 pixels high. Each char in a row
represents one, 8-pixel, vertical column of a character. 5 chars
per character. */
...
Each char in this array covers a 8-pixel vertical block on the
display. Each successive char covers the next 8-pixel column over
...
char displayMap[LCD_WIDTH * LCD_HEIGHT / 8] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // (0,0)->(11,7) ~ These 12 chars cover an 8x12 block in the left corner of the display
...
void LCDWrite(char data_or_command, char data)
{
  ...
  //Send the data
  digitalWrite(CE, LOW);
  SPI.transfer(data); //shiftOut(DIN, CLK, MSBFIRST, data);
...
// library.
void setLine(int x0, int y0, int x1, int y1, char bw)
{
...
// This function was grabbed from the SparkFun ColorLCDShield
// library.
void setRect(int x0, int y0, int x1, int y1, char fill, char bw)
{
...
// This function was grabbed from the SparkFun ColorLCDShield
// library.
void setCircle (int x0, int y0, int radius, char bw, int lineThickness)
{
...
{
  char column; // temp char to store character's column bitmap
  for (int i=0; i<5; i++) // 5 columns (x) per character
  {
    column = pgm_read_char(&ASCII[character - 0x20][i]);
...
void setBitmap(const char * bitArray)
{
  ...
  {
    char c = pgm_read_char(&bitArray[i]);
    displayMap[i] = c;
  }
}
