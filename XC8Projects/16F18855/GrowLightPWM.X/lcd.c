...
//    LCD_Write(LCD_DATA, 0x00); //Blank vertical line padding
    ...
    {
    LCD_Write(LCD_DATA, fonts[character - 0x20][index]); //0x20 is the ASCII character for Space (? ?). The font table starts with this character
    }
...
LCD_Write(LCD_COMMAND, 0xBB); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark

LCD_Write(LCD_COMMAND, 0x04); //Set Temp coefficent

LCD_Write(LCD_COMMAND, 0x14); //LCD bias mode 1:48: Try 0x13 or 0x14

LCD_Write(LCD_COMMAND, 0x20); //We must send 0x20 to tell the display to use standard commands
