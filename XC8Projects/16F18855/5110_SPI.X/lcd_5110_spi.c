...
{
    SPIWrite(0, 0x80 | x);                      // goto Column position "x".
    SPIWrite(0, 0x40 | y);                      // goto Row position "y".
}


void LCDBitmap(const char bmp_array[],  char startLine, int len)   // len is 84 x number of lines to display
{
    int index;
    gotoXY(0,startLine);                        // X is always 0, Y is line number tostart on
    for (index = 0 ; index < len ; index++)     // loop through the bmp_array,
    {
        SPIWrite(LCD_DATA, bmp_array[index]);   // & write it byte by byte.     
    } 
}
...
{
	int str[5]={0,0,0,0,0};                    // Integer can be up to 5 characters long
	signed char i=4,j=0;
...
//The acceptable range is between 0x80 (being a contrast value of 0) and 0xFF (being a contrast value of 127)
    ...
    SPIWrite(LCD_COMMAND, 0xBE);                //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark

    SPIWrite(LCD_COMMAND, 0x04);                //Set Temp coefficent

    SPIWrite(LCD_COMMAND, 0x13);                //LCD bias mode 1:48: Try 0x13 or 0x14
...
{
    char i; //,d;                            // Save 4 bytes by eliminating the d variable
...
void LCDDrawBox(void)
{
    ...
    {
        gotoXY(j, 0);
        SPIWrite(LCD_DATA,0x01);
    }

    for(j = 0; j < 84; j++)                     //Bottom
    {
        gotoXY(j, 5);
        SPIWrite(LCD_DATA,0x80);
    }
