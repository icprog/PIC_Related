...
#include "config.h"
#include "interface.h"		// ignored if parallel is selected
...
Noritake_VFD_CUU::nextLine() {
...
#if NORITAKE_VFD_CU20045_UW4J==1
		if (x < 0x20)
			x = 0x20;
		else if (x<0x40)
			x = 0x40;
		...
		if (x < 0x14) // line 0
			x = 0x40;
#if NORITAKE_VFD_LINES > 2
		else if (0x40<=x && x<0x54) // line 1
			x = 0x14;
#if NORITAKE_VFD_LINES > 3
...
Noritake_VFD_CUU::CUU_home() {
	CUU_command(0x02 + NORITAKE_VFD_FONT);
	_delay_us(MIN_DELAY);
}
...
Noritake_VFD_CUU::CUU_setCursor(uint8_t col, uint8_t line) {
	if (col < NORITAKE_VFD_COLS && line < NORITAKE_VFD_LINES) {
#if NORITAKE_VFD_CU20045_UW4J==1
		switch (line) {
		case 0:
